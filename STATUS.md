...
1. Build the Synthese package via /submission-gate: human-browser confirm of the live Synthese submission guidelines (Springer cookie-walled agent access on 2026-07-13), anonymized LaTeX manuscript + separate title page with acknowledgements, Editorial Manager submission. The JSO DOCX pipeline (`scripts/build_jso_submission.py`) is not needed for Synthese; do not overwrite `submission/jso/` (archived record of what JSO reviewed).
2. Pre-submission checklist carries two watch items from the venue record: the live-guidelines confirm above, and possible deeper natural-kinds literature coverage (Boyd beyond 1999, Millikan, Franklin-Hall) if a referee asks; not a blocker now.
3. At package time, rerun `.house-style/check-terms.py` against the Synthese reader split recorded in `submission/venue-decision-2026-07-13.md` (free: projectibility, intervention, grounding, etc.; earned: the social-ontology apparatus, all currently glossed).

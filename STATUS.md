...
1. Retarget via /venue-selection with a venue decision record in `submission/` before any target-specific files. The revision is framed philosophy-general (Synthese/Ergo/Phil Studies tier); confirm the chosen venue's readers own the debate §1 names, and adjust the §1 debate-placement paragraph if not.
2. Regenerate any future package with `python3 scripts/build_jso_submission.py` (section-numbering fix now included; the JSO-named paths/filenames need retargeting to the new venue first). Do not overwrite `submission/jso/`; it is the archived record of what JSO reviewed.
3. At package time, apply the apparatus-term gate by hand until portfolio tooling exists: every program-internal term (projectible, profile, anchoring, cascade, uptake, conferral) glossed at or before first use, abstract included.

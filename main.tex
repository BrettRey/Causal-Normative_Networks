...
The network is not a replacement for grounding. It is a representation of real dependence relations: the field-relative grounds of statuses, their correlative positions, the recognitions that fit or misfit them, and the uptake that supports social projection from those statuses. Epstein's frame principles and anchors explain why the field's status conditions are in place \citep{epstein2015AntTrap}; the network explains how those conditions combine with recognition and causal response.
...
The two reductions now fail from opposite sides. Causal order alone was too narrow; normative order alone is too thin. The explanatory unit is neither causal regularity nor normative status by itself, but a normative position coupled to causal uptake.\footnote{The dialectic is parallel to the rules-and-equilibria programme in social ontology: rules alone are too thin, behavioural equilibria alone are too thin, and institutions are better understood by representing both together \citep{hindriksGuala2015InstitutionsRulesEquilibria}.} Section~\ref{sec:misrecognition} turns to the cases in which the two come apart.
...
Contract law has parallel discharge devices, such as accord and satisfaction or a contract not to sue, that alter the obligation profile even if demand continues \citep[\S\S~281, 285]{americanLawInstitute1981RestatementContracts}.

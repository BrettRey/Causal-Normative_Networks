...
Causal inference distinguishes three levels of question: association asks what accompanies what, intervention asks what would follow if a node were set by fiat, and counterfactuals ask what would have held had things gone otherwise \citep{pearl2009}. The what-if patterns above are questions of the second and third levels, but only the causal edges answer to the do-operator; on transition and correlative structure, the answers are fixed by the field's rules rather than by mechanisms.
